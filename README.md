...
> **Work in progress.** Expect rough edges, missing features, and breaking changes. The camera scanner is at its first milestone (fixed-grid scanning); the learned face detector it's building toward doesn't exist yet.
...
The longer-term plan (see [MILESTONES.md](MILESTONES.md)) is a learned keypoint detector so you can just turn the cube in view — no grid alignment, no prompts. Design notes live in [CLAUDE.md](CLAUDE.md).
