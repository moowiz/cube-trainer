...
export class SolverClient {
  ...
  /** Solve the worker's log; with `fromT`, only the evidence sampled since that wall-clock ms (one epoch of a followed solve). */
  requestSolve(fromT?: number): Promise<Solution> {
    if (this.pending) throw new Error('SolverClient: a solve is already pending');
    const id = this.nextId++;
    return new Promise((resolve, reject) => {
      this.pending = { id, resolve, reject };
      const msg: SolverRequest = fromT === undefined ? { type: 'solve', id } : { type: 'solve', id, fromT };
      this.w.postMessage(msg);
    });
  }
