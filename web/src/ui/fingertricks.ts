...
const MOVE: Record<string, string> = {
  ...
  "U'": 'Top layer, front edge goes right. Right index flick: rest it on the front-right of the top layer and push away from you. Or the left index, hooked over the back-left corner, pulling toward you.',
  U2: 'Top layer half turn. Double flick: right index then middle finger in one motion, or two index flicks; the left hand does the same from the front-left. Either direction, whichever finger is free.',
  ...
  "D'": 'Bottom layer, its front goes left. Right ring finger under the back-right of the bottom layer pulling toward you; or the left ring finger under the back-left pushing it right, away from the hand.',
  D2: 'Bottom layer half turn. Two ring-finger pulls (right then left), or ring then little finger on one hand, either hand from its back corner.',
  B: 'Back layer, its top goes left (seen from the front). Right index over the top-back edge pushing left; or the left thumb on the back-left of the back layer pushing down; or regrip and turn it with the left hand from above.',
  "B'": 'Back layer, its top goes right. Left index over the top-back edge pushing right; or the right thumb on the back-right pushing down; or regrip.',
  B2: 'Back layer half turn. Two index pushes from the top, or a thumb push from the back-left then an index push; or regrip: in a scramble it is fine to take it from above with both hands.',
